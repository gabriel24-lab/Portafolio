    ...
    <>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button onClick={() => setLang(lang === "es" ? "en" : "es")} aria-label={lang === "es" ? "Cambiar idioma a inglés" : "Switch language to Spanish"} style={{ background: dark ? "#1c1c26" : "#f1f0ed", border: `1px solid ${border}`, borderRadius: 10, padding: "6px 12px", color: text, fontFamily: "'Play', sans-serif", fontSize: 12, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}>{lang === "es" ? "EN" : "ES"}</button>
          <button onClick={() => setDark(!dark)} aria-label={dark ? "Cambiar a modo claro" : "Cambiar a modo oscuro"} style={{ background: dark ? "#1c1c26" : "#f1f0ed", border: `1px solid ${border}`, borderRadius: 10, width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 18, transition: "all 0.2s", color: text }}><i className={dark ? "bi bi-sun" : "bi bi-moon-stars"} /></button>
          <button onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? "Cerrar menú" : "Abrir menú"} aria-expanded={menuOpen} style={{ background: dark ? "#1c1c26" : "#f1f0ed", border: `1px solid ${border}`, borderRadius: 10, width: 38, height: 38, display: "none", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 18, color: text }} className="hamburger"><i className={menuOpen ? "bi bi-x-lg" : "bi bi-list"} /></button>
        </div>
